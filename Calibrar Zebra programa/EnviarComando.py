...
COMMAND_PATH = r'C:\Program files (x86)\TreHoffman Technologies\RAW Print\command.txt'
    ...
    if process_status("Calibrar_Zebra.exe") <= 2:
        ...
        try:
            with open(COMMAND_PATH, "r") as COMMAND_FILE:
                command = COMMAND_FILE.readlines()
        except FileNotFoundError:
            with open(COMMAND_PATH, "w") as COMMAND_FILE:
                COMMAND_FILE.write("^XA~JC^XZ")
                COMMAND_FILE.close()
                command = "^XA~JC^XZ"

                
        for i,com in enumerate(command):
            command[i] = com.replace("\n","")
